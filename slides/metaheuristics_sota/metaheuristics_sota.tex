...
\item<2-> \alert{Baselines}:\uncover<2->{%
\begin{itemize}%
\item \gls{algoRLS} with a unary operator that swaps two elements\uncover<3->{,}%
\item<3-> \gls{algoRLS} with a unary operator that swaps a binomially distributed number of elements~(similar to the \gls{algoEAopo} for permutations)\uncover<4->{,}%
\item<4-> \gls{algoSA} with the same operators and suitable exponential temperature schedule setups\uncover<5->{,}%
\item<5-> \gls{algoTS} with the same operator that forbids moving values to recently-used indicess\uncover<6->{, and a}%
\item<6-> \gls{algoEAmpl} with these operators and maybe \glsFull{algoAPcross}\cite{LKPM1997DBNTOTMGWGA,LKMID1999GAFTTSPARORAO}.%
\end{itemize}%
}%
...
%
\item<2-> The \glsShort{optTSP} has many applications in transportation, logistics, drilling problems, telescope aiming, and genome sequencing\cite{ABCC2006TTSPACS}.%
%
\item<3-> \alert{Benchmarks}: \uncover<3->{%
\begin{itemize}%
\item The \gls{TSPLib}\cite{R2016T,R1991ATSPL}\only<-3>{ %
...
\item<4-> instances of the 8th~DIMACS challenge\cite{JMGR20008DICTTSP,JM2002EAOHFTS,JGMYZZ2002EAOHFTS}\only<-4>{, %
with instances with up to~10\decSep000\decSep000~cities.%
}%
\uncover<5->{, the}%
\item<5-> \citetitle{C2013WT} repository\cite{C2013WT}\only<-5>{ %
...
\item<6-> \gls{TSPSuite}\only<-6>{, which offers an integrated \gls{Java} environment for benchmarking algorithms on the \gls{TSPLib} instances}\cite{WCLTTCMY2014BOAAOSFFTTSP}.
\end{itemize}%
}%
%
\item<7-> \alert{Baselines}: the \only<-7>{\glsFull{algoLK} algorithm, which exchanges variable number of edges in a tour.}\only<8->{\glsShort{algoLK} algorithm}\cite{LK1973AEHAFTTSP}%
%
\item<8-> \alert{\gls{SOTA}}:\uncover<8->{%
\begin{itemize}%
\item the \only<-8>{%
\glsFull{algoLKH} algorithm building on top of the \glsShort{algoLK} algorithm.}\only<8->{\glsShort{algoLKH} algorithm}\cite{H2009GKOSFTLKTH,H2000AEIOTLKTSH}\only<9->{,}%
%
\item<9-> \only<-9>{an \glsShort{algoEA} with \glsFull{algoEAX} as binary operator}\only<10->{\glsShort{algoEAX}}\cite{NK1997EACAHPGAFTTSP,NS2012ANGAFTATSP,NK2013APGAUEACFTTSP}\only<10->{,}%
%
\item<9-> \only<-10>{an \glsShort{algoMA} with \glsFull{algoGPX} as binary operator}\only<11->{\glsShort{algoGPX}}\cite{WHH2010AHGAFTTSPUGPC}\only<11->{, in particular}%%
%
\item<11-> \only<-11>{\glsShort{algoLKH} run in an \glsShort{algoMA}-fashion with \glsShort{algoGPX} as binary operator}\only<12->{\glsShort{algoLKH} + \glsShort{algoGPX}}\cite{WHH2010AHGAFTTSPUGPC}\only<12->{, and}%%
%
\item<12-> \gls{Concorde}, an exact solver for the \gls{optTSP}\cite{C1997CTS,ABCC2006TTSPACS,C2012IPOTTSMATLOC}, which could even solve the largest instance of \gls{TSPLib}~(with 85\decSep900~cities) to optimality!%
%
\end{itemize}%
}%
%

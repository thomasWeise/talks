...
\begin{itemize}%
%
\item \alert{Problem}: Search space~\searchSpace\ corresponds to the set~$\{0,1\}^n$ of bit strings of length~$n$ and nothing is known about objective~function~$\objFun:\{0,1\}^n\mapsto\naturalNumbersZ$.%
%
...
%
\only<9->{, \only<-9>{\alert<9>{\glsShort{optPlateau}}}\only<10->{\glsShort{optPlateau}}\cite{AD2018PRAFP}\only<-9>{ (like Jump, but bits around optimum are neutral)}}%
%
...
%
\only<11->{, \only<-11>{\alert<11>{\glsShort{optNQueens}}}\only<12->{\glsShort{optNQueens}}\cite{DYHWSB2020BDOHWI}\only<-11>{~(place $n$~queens on chessboard, many interactions between variables)}}%
%
...
\begin{itemize}%
\item \alert{Problem}: \glsFullpl{optLABS} are bit strings of length~$n$ whose autocorrelation is as low as possible. %
%
\item<2-> This means that sub-sequences of any length~$1\leq k<n$ should ideally never repeat. %
\item<3-> The objective function~$\objFun:\{0,1\}^n\mapsto\naturalNumbersZ$ represents the energy of a bit string~\sespel\ and can be defined as~$\objFunb{\sespel}=\sum_{k=1}^{n-1}\left(\sum_{i=0}^{n-1-k} \arrayIndex{s}{i} \cdot \arrayIndex{s}{i+k}\right)^2$ with~$\arrayIndex{s}{j}=2\arrayIndex{\sespel}{j}-1$.%
...
%
\item<5-> \alert{Benchmarks}: The objective function for different values of~$n$%
%
\item<6-> \alert{Baselines}: see discrete optimization\uncover<7->{ and for small~$n$ %
...
\begin{itemize}%
\item \alert{Problem}: The search space~\searchSpace\ corresponds to the set of all permutations of the first $n$~natural numbers.%
...
\gls{algoSA} with the same operators and suitable exponential temperature schedule setups\uncover<5->{, and a %
\gls{algoEAmpl} with these operators and the Alternating Position Crossover~(AP) operator\cite{LKPM1997DBNTOTMGWGA,LKMID1999GAFTTSPARORAO}.%
}}}%
\item<4-> \alert{\gls{SOTA}}:%
\end{itemize}%
\end{frame}%

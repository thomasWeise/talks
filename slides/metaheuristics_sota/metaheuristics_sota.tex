...
\item<4-> Such sequences are needed for communication- and radar-like applications.%
\item<5-> \alert{Benchmarks}: The objective function for different values of~$n$%
%
\item<6-> \alert{Baselines}: see discrete optimization, a binary-coded \glspl{algoEA}\cite{MZB1998ESFLABS}, or exhaustive enumeration\cite{M1996ESFLABS}~(for small~$n$)
%
%
\item<5-> \alert{\gls{SOTA}}:\uncover<5->{%
\begin{itemize}%
\item<5-> the somewhat outdated online repository~\cite{BBB2016AGAFSASOTLP},%
\item<6-> for small~$n$ maybe the \gls{algoMA} from\cite{GCF2007AMAFTLABSP}, the local search in\cite{HYH2008ESLSFTLABSP}, or the branch-and-bound method from\cite{P2013IBABFLABS}.%
\end{itemize}%%
}%
\end{itemize}%
\end{frame}%

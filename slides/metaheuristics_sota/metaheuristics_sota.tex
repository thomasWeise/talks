...
\begin{itemize}%
\item Today, we will discuss the research frontier on metaheuristic optimization.%
...
\only<-13>{\glsFull{algoRLS}~(unary operator flips 1~bit)}\only<14->{\gls{algoRLS}}%
%
\only<14->{, %
...
%
\item<16-> \alert{\gls{SOTA}}: %
\only<-16>{\alert<16>{\glsShort{algoSAGA}}}\only<17->{\glsShort{algoSAGA}}\cite{CPD2018TAMPARAOEA,DD2018OSASAPCFT1LLGA}%
\only<-16>{~(linear runtime on \glsShort{optOneMax})}%
%
\only<17->{, %
\only<-17>{\alert<17>{\glsShort{algoG2P1GA}}}\only<18->{\glsShort{algoG2P1GA}}\cite{S2012CSUBBA,S2017HCSUBBAIGA,CPD2018TAMPARAOEA,CPD2018ASPFTUOCIBBO}%
\only<-17>{~(algorithm using binary operator and being twice as fast as any unary-operator only black-box method on \glsShort{optOneMax})}%
}%
%
\only<18->{, and the %
\only<-18>{\alert<18>{\glsFull{algoFEAopo}}}\only<19->{\glsShort{algoFEAopo}}\cite{WWLC2021FFAMOAIUBTOTOFV,WWLCL2023FFAOWBFGSCBE,ZWWW2026WFFAFTSIFGLS}%
\only<-19>{~(the only algorithm achieving polynomial runtime on \glsShort{optTrap}, \glsShort{optTwoMax}, and \glsShort{optJump})}.%
}%
%
\end{itemize}%
%

...
\end{frame}%
%
\section{Problems that algorithms cannot solve both efficiently \emph{and} exactly}%
%
...
%
\begin{itemize}%
\item In an \glsFull{optTSP}, we want to find the shortest path through \scale~locations and back to the start\cite{ABCC2006TTSPACS,LLRKS1985TTSPAGTOCO,GP2002TTSPAIV,WCLTTCMY2014BOAAOSFFTTSP,LWLvdBTW2024ATTSPWFFAAHA,WWCTL2016GVLSTIOPSOEAP}.%
\end{itemize}%
%
...
%
\item<3-> If we have \scale~cities, then there are $1*2*3*\dots*(\scale-1)*\scale=\factorial{\scale}$ tours.%
%
\item<5-> Factorial growth~(\factorial{\scale}) is even worse than exponential growth\dots%
%
...
\begin{center}%
\resizebox{0.77\paperwidth}{!}{\parbox{0.5\paperwidth}{\noindent%
There are many more principles and problems surrounding (\gls{metaheuristic}) optimization.\uncover<4>{\\[10pt]%
...
%
\item<2-> For this, we actually use principles similar to the \gls{algoAstar}, just a bit differently, in methods\cite{ABCC2006TTSPACS,CEG2005CWDPIFT} like \glsFull{algoBB}\cite{LMSK1963AAFTTSP}.%
%
...
%
\item<4-> For this, we use algorithms a bit similar to \glsFull{algoRLS}, but with more targeted search steps, e.g., in the \glsFull{algoLKH} algorithm\cite{H2009GKOSFTLKTH,H2000AEIOTLKTSH}.%
%
\end{itemize}%
\end{frame}%
%

...
%
\item<2-> The search space~\searchSpace\ would then be the \scale-dimensional real vectors, i.e., $\searchSpace=\realNumbers^{\scale}$.%
%
\item<3-> The objective function would, well, be the function~\objFun.%
%
\item<4-> The optimal solution~$\globalOptimumOf{\sespel}\in\searchSpace$ is the minimum of~\objFun.%
%
...
\begin{itemize}%
%
\item Because of this hardness of optimization, \gls{metaheuristic} algorithms that follow the trial-and-error idea of iterative improvement have emerged.%
...
\begin{itemize}%
\item Local search with $|S_i|=|N_i|=1$ is the simplest realization of the \gls{metaheuristic} idea.%
...
\begin{itemize}%
%
\item Discrete optimization algorithms work on bit strings of length~\scale\ as search space and use only the objective values of the solutions they sample to solve a problem.%
\end{itemize}%
%
...
%
\item<4-> The \gls{algoEAopo} can easily solve the \gls{optOneMax} problem, where the goal is to find a bit string of all~\texttt{1}s\cite{M1992HGARWMAH}.%
%
\item<5-> It needs \bigThetab{n^n} on traps\cite{DJW2002OTAOTOPOEA}, where the optimum and the worst solution are exchanged compared to \gls{optOneMax}\only<-5>{.}\uncover<6->{, as well as on \gls{optTwoMax}\cite{FQW2018ELDBOAWHTMO,VHGN2002FTTIMEAHSP}, which has one local and one global optimum.}%
\end{itemize}}%
\uncover<7->{%
\begin{itemize}%
\item The \gls{algoFEAopo} solves all three problems in polynomial time\only<-7>{!}\uncover<8->{ as well as the \gls{optJump} problem, where the \pgls{algoEAopo} also needs exponential runtime!}%
\end{itemize}}}%
%

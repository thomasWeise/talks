...
\begin{itemize}%
%
\item In the \glsFull{optTSP}\cite{ABCC2006TTSPACS,LLRKS1985TTSPAGTOCO,GP2002TTSPAIV,WCLTTCMY2014BOAAOSFFTTSP,LWLvdBTW2024ATTSPWFFAAHA,WWCTL2016GVLSTIOPSOEAP}, the goal is to find the shortest round-trip tour through a set of \scale~cities.%
%
\item<2-> The search space \searchSpace\ thus is the set of all possible round-trip tours through these \scale~cities, usually specified as permutations of the first \scale~natural numbers.%
%
\item<3-> The objective function~$f:\searchSpace\mapsto\realNumbers$, subject to minimization, is the length of the tour.%
%
\item<4-> The optimal solution~$\globalOptimumOf{\sespel}\in\searchSpace$ is the shortest possible tour.%
%
\end{itemize}%
}%
%
...
\end{frame}%
%
\xdef\sharedSlideTitle{Example: Maximum Satisfiability Problem}%
\input{\sharedDir/slides/optimization_maxsat.tex}%
%
\begin{frame}[t]%
\frametitle{Example: Bin Packing Problem}%
\begin{itemize}%
\item The goal of the Bin Packing Problem is to pack \scale~objects, each having a specific size, into as few bins~(also of a given size) as possible\cite{ZLWvdBTW2024RLSOT2RBPPWIR,ZWvdBTLTW2024RLSFTDBPAANRFFFA}.%
\item<2-> The \searchSpace\ comprises all possible packing orders of the \scale~objects.%
\item<3-> The objective function~\objFun\ is the number of bins needed by a given packing order.%
\item<4-> The optimum~$\globalOptimumOf{\sespel}$ is the packing order requiring the fewest bins.%
\end{itemize}%
...
\begin{itemize}%
%
\item Finding the globally optimal solution~\globalOptimumOf{\sespel} from the set of all possible solutions~\searchSpace\ is often an \npHard\ problem.%
%
...
%
\item<4-> In other words, if we want to guarantee to find the best possible solution~\globalOptimumOf{\sespel} for all possible instances of a problem, we often cannot really be much faster than testing all possible candidate solutions~$\sespel\in\searchSpace$ in the \alert{worst case}.%
%
\end{itemize}%
%
...
\begin{itemize}%
\item What we have learned is that different \glspl{metaheuristic} realize the trial-and-error scheme differently.%
...
%
\item<4-> For the \gls{algoEAopo}, $P=1$ if~$\objFunb{\sespel_n}\leq \objFunb{\sespel_c}$ and~$0$~otherwise.%
%
\item<5-> In the \gls{algoSA}, $P$~depends on the difference~$\Delta=\objFunb{\sespel_c}-\objFunb{\sespel_n}$ and is~$1$~if $x_n$ is better than~$x_c$.%
%
\item<6-> In an \gls{algoSGA} for maximization, $P$~is~$\objFunb{\sespel_n}/(\objFunb{\sespel_c}+\objFunb{\sespel_n})$.\uncover<7->{ If we wanted to do minimization, maybe we could use ${\color{orange}{1-}}\objFunb{\sespel_n}/(\objFunb{\sespel_c}+\objFunb{\sespel_n})$ instead.}%
%
...
%
\begin{frame}%
\frametitle{Invariance Properties: \gls{optOneMax} Example}%
\parbox{0.415\paperwidth}{%
\begin{itemize}%
\item \gls{optOneMax} is the simplest benchmark problem in discrete optimization\cite{DJW2006UALBFRSHIBBO,M1992HGARWMAH,ZWWW2026WFFAFTSIFGLS}.%
\item<2-> It is defined over~$\searchSpace=\{0,1\}^{\scale}$, i.e., the bit strings of length~\scale.%
\item<3-> Goal:~\inQuotes{Find the bit string of all 1s!}%
\item<4-> \inQuotes{Maximize the number of 1s!}%
\item<5-> $\fn{1}(x)=\scale-\sum{x}$~(for minimization).%
...
%
\begin{frame}%
\frametitle{Invariance Properties: \gls{optOneMax} Example -- Translation}%
\parbox{0.415\paperwidth}{%
\begin{itemize}%
...
%
\item<8-> $\fn{1}(\sespel_n)=14$\uncover<9->{, $\fn{1}(\sespel_c)=6$\uncover<10->{, $\fn{2}(\sespel_n)=24$\uncover<11->{, $\fn{2}(\sespel_c)=16$}}}%
...
%
\begin{frame}%
\frametitle{Invariance Properties: \gls{optOneMax} Example -- Scaling}%
\parbox{0.415\paperwidth}{%
\begin{itemize}%
...
%
\item<5-> $\fn{1}(\sespel_n)=14$, $\fn{1}(\sespel_c)=6$\uncover<6->{, $\fn{3}(\sespel_n)=11.2$, $\fn{3}(\sespel_c)=4.8$}%
%
...
%
\begin{frame}%
\frametitle{Invariance Properties: \gls{optOneMax} Example -- Squaring}%
\parbox{0.415\paperwidth}{%
\begin{itemize}%
\item Now I create another slightly modified variant of this problem.%
...
%
\item<4-> $\fn{1}(\sespel_n)=14$, $\fn{1}(\sespel_c)=6$\uncover<5->{, $\fn{4}(\sespel_n)=196$, $\fn{4}(\sespel_c)=36$}%
%
...
%
\begin{frame}%
\frametitle{Invariance Properties: \gls{optOneMax} Example -- \gls{optTrap}}%
\parbox{0.415\paperwidth}{%
\begin{itemize}%
\item Now I create another modified variant of this problem:~a \gls{optTrap}\cite{DJW2002OTAOTOPOEA,NB2003AAOTBOSEAOTF}.%
...
\item<6-> The \gls{algoEAopo} has expected runtime~\bigThetab{\scale^{\scale}} on \glspl{optTrap}\cite{DJW2002OTAOTOPOEA}.%
\end{itemize}%
}%
...
%
\begin{frame}%
\frametitle{Invariance Properties: \gls{optOneMax} Example -- \gls{optJump}}%
\parbox{0.415\paperwidth}{%
\begin{itemize}%
...
\item<5-> The \gls{algoEAopo} has expected runtime~\bigThetab{\scale^{\omega} + \scale\ln{\scale}} on jumps\cite{DJW2002OTAOTOPOEA}.%
\end{itemize}%
}%
...
%
\begin{frame}%
\frametitle{Invariance Properties: \gls{optOneMax} Example -- Injection}%
\parbox{0.415\paperwidth}{%
\begin{itemize}%
...
\item<8-> \alert{They perform identical on ALL of the \gls{optOneMax}-based functions from before!}%
\end{itemize}%
\end{frame}%
%
\begin{frame}[t]%
...
%
\begin{frame}%
\frametitle{(1+1)~FEA on \gls{optOneMax}}%
\parbox{0.37\paperwidth}{%
\begin{itemize}%
\item As first example, we again take \gls{optOneMax}\cite{M1992HGARWMAH}.%
\item<2-> The expected runtime of \gls{algoEAopo} on \gls{optOneMax} is in \bigThetab{\scale\ln{\scale}}\cite{DDE2015FBBCTDNGA,M1992HGARWMAH}.%
\item<3-> The average runtime \gls{algoFEAopo} on \gls{optOneMax} seems to be slower by factor proportional to~\scale, i.e., seems to be in~\bigOb{\scale^2\ln{\scale}}\cite{WWLC2021FFAMOAIUBTOTOFV}.%
\end{itemize}%
}%
%
...
%
\begin{frame}%
\frametitle{(1+1)~FEA on \gls{optLeadingOnes}}%
\parbox{0.37\paperwidth}{%
\begin{itemize}%
\item The \gls{optLeadingOnes} problem maximizes the number of leading \texttt{1} bits in a bit string\cite{W1989TGAASPWRBAORTIB,R1997CPOEA}.%
\item<2-> The expected runtime of \gls{algoEAopo} on \gls{optLeadingOnes} is in \bigThetab{\scale^2}\cite{DJW2002OTAOTOPOEA}.%
\item<3-> The average runtime \gls{algoFEAopo} on \gls{optLeadingOnes} seems to be slower by factor proportional to~\scale, i.e., seems to be in~\bigOb{\scale^3}\cite{WWLC2021FFAMOAIUBTOTOFV}.%
\end{itemize}%
}%
%
...
%
\begin{frame}%
\frametitle{(1+1)~FEA on \gls{optTwoMax}}%
\parbox{0.37\paperwidth}{%
\begin{itemize}%
\item The \gls{optTwoMax} problem has one local opposite to a global optimum\cite{FQW2018ELDBOAWHTMO,VHGN2002FTTIMEAHSP,ZWWW2026WFFAFTSIFGLS}.%
\item<2-> An \gls{algoEAopo} has a 50/50 chance to converge to the local or global optimum and thus has an expected runtime in \bigThetab{\scale^{\scale}}.\cite{FQW2018ELDBOAWHTMO,FOSW2009AODPMFGE}%
\item<3-> The average runtime \gls{algoFEAopo} on \gls{optTwoMax} seems to be in~\bigOb{\scale^2 \ln{\scale}}\cite{WWLC2021FFAMOAIUBTOTOFV}.%
\end{itemize}%
}%
%
...
%
\begin{frame}%
\frametitle{(1+1)~FEA on \gls{optTrap}}%
\parbox{0.37\paperwidth}{%
\begin{itemize}%
\item The \gls{optTrap} problem basically swaps the global optimum with the worst solution\cite{DJW2002OTAOTOPOEA,NB2003AAOTBOSEAOTF}.%
\item<2-> An \gls{algoEAopo} will converge to the resulting local optimum and thus have an expected runtime of \bigThetab{\scale^{\scale}}\cite{DJW2002OTAOTOPOEA}.%
\item<3-> The average runtime \gls{algoFEAopo} on \gls{optTrap} seems to be in~\bigOb{\scale^2 \ln{\scale}}\cite{WWLC2021FFAMOAIUBTOTOFV}.%
\end{itemize}%
}\\%
\uncover<4->{\begin{itemize}%
\item The expected runtime is the same as on \gls{optOneMax}, because \gls{optTrap} is an injective transformation of \gls{optOneMax}, symmetric, and FFA \inQuotes{converges} into both directions.%
\end{itemize}%
}%
%
...
%
\begin{frame}%
\frametitle{(1+1)~FEA on \glspl{optJump}}%
\parbox{0.37\paperwidth}{%
\begin{itemize}%
\item The \gls{optJump} problem inserts a deceptive region of~$\omega-1$~bits before the optimum, forcing the algorithm to toggle~$\omega$ bits at once\cite{DJW2002OTAOTOPOEA,FQW2018ELDBOAWHTMO}.%
\item<2-> An \gls{algoEAopo} will converge to the resulting local optimum and thus have an expected runtime of \bigThetab{\scale^{\omega}+\scale\ln{\scale}}\cite{DJW2002OTAOTOPOEA}.%
\item<3-> The average runtime \gls{algoFEAopo} on \gls{optJump} seems to \alert{always} be in~\bigOb{\scale^2 \ln{\scale}}\cite{WWLC2021FFAMOAIUBTOTOFV}.%
\end{itemize}%
}\\%
\uncover<4->{\begin{itemize}%
\item It is \alert{identical} to its performance on \gls{optOneMax} regardless of~$\omega$, because all \glspl{optJump} are an injective transformation of \gls{optOneMax}!%
\end{itemize}%
}%
%
...
%
\begin{frame}%
\frametitle{(1+1)~FEA on \glspl{optPlateau}}%
\parbox{0.37\paperwidth}{%
\begin{itemize}%
\item The \gls{optPlateau} problem inserts a neutral region of~$\omega-1$~bits before the optimum, forcing the algorithm to toggle~$\omega$ bits at once\cite{AD2018PRAFP}.%
\item<2-> An \gls{algoEAopo} has an expected runtime of \bigThetab{\scale^{\omega}} on plateaus\cite{AD2018PRAFP}.%
\item<3-> The average runtime \gls{algoFEAopo} on \glspl{optPlateau} is slower, again by a factor seemingly proportional in~\scale.%
...
%
\begin{frame}%
\frametitle{(1+1)~FEA on \gls{optMaxSAT}}%
\begin{itemize}%
\item The \glsFull{optMaxSAT}\cite{HS2005SLSFAA,C1971TCOTPP} problem is \npComplete~\cite{C1971TCOTPP}.%
\item<2-> \gls{optSATLIB}\cite{HS2000SAORFROS} provides satisfiable benchmark instances from the phase transition region~(i.e., the hardest type of instances) for different scales~$\scale\in20 \cup \{25 i\; \forall i \in \intRange{2}{10}\}$.%
...
\uncover<2->{%
\begin{itemize}%
\item Let’s say you have an optimization problem with objective function~\objFunb{x}.%
\item<3-> You \alert{encrypt} the objective values and do not tell them to the algorithm.%
\item<4-> Let's say you apply \glslink{cryptoAES}{AES}\cite{FIPSPUB197,FIPSPUB197upd1}, \gls{cryptoRSA}\cite{RSA1978AMFODSAPKC,Y2006MCFCTR}, or the Caesar cipher\cite{O2002CACJCTEATI,Y2006MCFCTR} as a function~$g:\naturalNumbersZ\mapsto\naturalNumbersZ$ to~\objFun, i.e., do~$g\circ \objFun$.%
\item<5-> Encryption removes any order, correlation or causality information, i.e., $g\circ \objFun$ does not correlate with~\objFun\ in any way.%
\item<6-> $g(\objFunb{x})$~will be totally random.\uncover<7->{ No reasonable optimization algorithm could work with that.}%
\item<8-> If the \gls{algoFEAopo} can find the optimum of~\objFunb{x}\dots%
\item<9-> {\dots}then it will find exactly the same solution in exactly the same runtime even if you apply it to the encrypted problem~$g(\objFunb{x})$!%
...
\item<5-> FFA can get trapped too\only<-5>{!}\uncover<6->{:~If all neighbors that can be reached with one search step around one solution all have the same objective value, then FFA-guided search may not be able to escape from that solution\cite{ZWWW2026WFFAFTSIFGLS}.\uncover<7->{ This can happen with 1-bit flips on \gls{optOneMax}\cite{ZWWW2026WFFAFTSIFGLS}.}%
}%
\end{itemize}%
}%
\end{itemize}%

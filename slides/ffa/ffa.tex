...
%
%
\DeclareCiteCommand{\ffaWorkInternal}%
{\usebibmacro{prenote}}%
{%
\clearfield{urlyear}\clearname{urlyear}\clearlist{urlyear}%
\clearfield{doi}\clearname{doi}\clearlist{doi}%
\clearfield{issn}\clearname{issn}\clearlist{issn}%
\clearfield{isbn}\clearname{isbn}\clearlist{isbn}%
\clearfield{editor}\clearname{editor}\clearlist{editor}%
\clearfield{address}\clearname{address}\clearlist{address}%
\clearfield{location}\clearname{location}\clearlist{location}%
\clearfield{eventdate}\clearname{eventdate}\clearlist{eventdate}%
\clearfield{venue}\clearname{venue}\clearlist{venue}%
...
\clearfield{publisher}\clearname{publisher}\clearlist{publisher}%
\usedriver{%
}{\thefield{entrytype}}%
}%
{\multicitedelim}%
...
%
\protected\gdef\ffaWork#1{{\small{\ffaWorkInternal{#1}}}}%
%
...
\begin{itemize}%
%
\item In the \glsFull{optTSP}\cite{ABCC2006TTSPACS,LLRKS1985TTSPAGTOCO,GP2002TTSPAIV,WCLTTCMY2014BOAAOSFFTTSP}, the goal is to find the shortest round-ttrip tour through a set of $n$~cities.%
%
...
%
\locateGraphic{}{width=0.75\paperwidth}{\sharedDir/graphics/fea_performance/fea_maxsat_ecdf}{0.125}{0.15}%
%
\end{frame}%
%
\begin{frame}%
...
\begin{itemize}%
\item We applied \gls{algoFFA} to the \glsFull{optMaxSAT}\uncover<2->{:%
\begin{itemize}%
\item \ffaWork{WWLC2021FFAMOAIUBTOTOFV}%
\end{itemize}%
}%
\end{itemize}%
\end{frame}%
...
\begin{itemize}%
\item We applied \gls{algoFFA} to the \glsFull{optTSP}\uncover<2->{:%
\begin{itemize}%
\item \ffaWork{LWLvdBTW2024ATTSPWFFAAHA}%
\item \ffaWork{LWLvdBW2022STTSPUFFA}%
\item \ffaWork{LWTWW2024GSIWIFFTTSP}%
\end{itemize}%
}%
\end{itemize}%
\end{frame}%
...
\begin{itemize}%
\item We applied \gls{algoFFA} to the \glsFull{optQAP}\uncover<2->{:%
\begin{itemize}%
\item \ffaWork{CWTW2024FFAOWBFGSORLSOTQAP}%
\item \ffaWork{TOvdBLW2024ESTAEFFFA}%
\end{itemize}%
}%
\end{itemize}%
\end{frame}%
...
\begin{itemize}%
\item We applied \gls{algoFFA} to the \glsFull{optTTP}\uncover<2->{:%
\begin{itemize}%
\item \ffaWork{XWvdBW2024RLSVNVFFAOTTP}%
\end{itemize}%
}%
\end{itemize}%
\end{frame}%

...
\item<6-> As a complement to this lesson, I suggest the report \emph{``Benchmarking in Optimization: Best Practice and Open Issues''}\cite{BBDvdBBCEFKLCLIMMNOVWW2020BIOBPAOI} on arXiv.%
\end{itemize}%
\end{frame}%
%
%
...
%
\item<2-> Let's look at the classical Traveling Salesperson Problem (TSP).%
\only<3-5,20->{%
\begin{itemize}%
...
\only<18->{Finding the best tour (what exact algorithms do) may take too long.}}%
\only<-20>{\item<19-> But we can find just \alert{some} tour very quickly.}%
\only<-21>{\item<20-> Of course the quality of that tour will be lower\only<21>{: the tour will be longer than the best one}.}%
\only<-23>{\item<22-> Is there something in between?}%
\only<-24>{\item<23-> (Meta-)Heuristic optimization algorithms try to find solutions which are as good as possible as fast as possible.}%
\item<24-> \alert{Optimization often means to make a trade-off between solution quality and runtime.}%
\end{itemize}%
}%
\end{itemize}%
%
...
%
%
\locateGraphic{2-3}{width=0.5\paperwidth}{graphics/tsp_china/tsp_china_solution}{0.25}{0.29}%
\locateGraphic{4-5}{width=0.58\paperwidth}{graphics/runtime_quality_tradeoff/runtime_quality_tradeoff_1}{0.21}{0.4}%
%
\locateGraphic{6}{width=0.8\paperwidth}{graphics/function_growth/function_growth_01}{0.1}{0.27}%
\locateGraphic{7}{width=0.8\paperwidth}{graphics/function_growth/function_growth_02}{0.1}{0.27}%
\locateGraphic{8}{width=0.8\paperwidth}{graphics/function_growth/function_growth_03}{0.1}{0.27}%
\locateGraphic{9}{width=0.8\paperwidth}{graphics/function_growth/function_growth_04}{0.1}{0.27}%
\locateGraphic{10}{width=0.8\paperwidth}{graphics/function_growth/function_growth_05}{0.1}{0.27}%
\locateGraphic{11}{width=0.8\paperwidth}{graphics/function_growth/function_growth_06}{0.1}{0.27}%
\locateGraphic{12}{width=0.8\paperwidth}{graphics/function_growth/function_growth_07}{0.1}{0.27}%
\locateGraphic{13}{width=0.8\paperwidth}{graphics/function_growth/function_growth_08}{0.1}{0.27}%
\locateGraphic{14}{width=0.8\paperwidth}{graphics/function_growth/function_growth_09}{0.1}{0.27}%
\locateGraphic{15}{width=0.8\paperwidth}{graphics/function_growth/function_growth_10}{0.1}{0.27}%
\locateGraphic{16}{width=0.8\paperwidth}{graphics/function_growth/function_growth_11}{0.1}{0.27}%
\locateGraphic{17}{width=0.8\paperwidth}{graphics/function_growth/function_growth}{0.1}{0.27}%
%
\locateGraphic{18}{width=0.58\paperwidth}{graphics/runtime_quality_tradeoff/runtime_quality_tradeoff_2}{0.21}{0.4}%
\locateGraphic{19}{width=0.58\paperwidth}{graphics/runtime_quality_tradeoff/runtime_quality_tradeoff_3}{0.21}{0.4}%
\locateGraphic{20-21}{width=0.58\paperwidth}{graphics/runtime_quality_tradeoff/runtime_quality_tradeoff_4}{0.21}{0.4}%
\locateGraphic{22}{width=0.58\paperwidth}{graphics/runtime_quality_tradeoff/runtime_quality_tradeoff_5}{0.21}{0.4}%
\locateGraphic{23}{width=0.58\paperwidth}{graphics/runtime_quality_tradeoff/runtime_quality_tradeoff_6}{0.21}{0.4}%
\locateGraphic{24}{width=0.58\paperwidth}{graphics/runtime_quality_tradeoff/runtime_quality_tradeoff_7}{0.21}{0.4}%
\locateGraphic{25}{width=0.58\paperwidth}{graphics/runtime_quality_tradeoff/runtime_quality_tradeoff_8}{0.21}{0.4}%
\end{frame}%
%
...
\item<2-> \only<4->{Two v}\only<-3>{V}iew\only<-3>{s}\cite{HAFR2010RPBBOB2ES,NFR2011CCCOTD,WNT2010AAOAB,WWQLT2018ADCOAAPIBAWATCFEDASAIF}\uncover<3->{:%
\begin{enumerate}%
...
\end{itemize}%
%
\locateGraphic{3}{width=0.4\paperwidth}{graphics/runtime_quality_tradeoff/runtime_quality_tradeoff_quality_after_runtime}{0.575}{0.55}%
\locateGraphic{4-}{width=0.4\paperwidth}{graphics/runtime_quality_tradeoff/runtime_quality_tradeoff_runtime_for_quality}{0.575}{0.55}%
%
\end{frame}%
%
\begin{frame}%
...
Measure the (absolute) consumed runtime of the \alert<2>{algorithm\only<2->{ implementation}} in~ms.%
\uncover<3->{%
\begin{itemize}%
%
\item \textcolor{green}{Advantages}\uncover<4->{:%
\begin{itemize}%
\item Results in many works reported in this format%
\item<5-> A quantity that makes physical sense%
\item<6-> Includes all ``hidden complexities'' of an algorithm implementation%
\end{itemize}%
}%
%
...
\item<10-> Can be biased by ``outside effects,'' e.g., OS, scheduling, other processes, I/O, swapping, \dots%
\end{itemize}%
}%
%
...
\begin{itemize}%
\item Results in many works reported in this format (or FEs can be deduced)%
...
\item<10-> 1~FE: very different costs in different situations!\cite{WCLTTCMY2014BOAAOSFFTTSP}\uncover<11->{%
\begin{itemize}%
\item When applying a local search that swaps two cities in each move to the Traveling Salesperson Problem (TSP), one FE can be done in $\bigOb{1}$.
\item<12-> When applying Ant Colony Optimization instead, each FE takes~$\bigOb{n^2}$.%
\end{itemize}%
}%
\end{itemize}%
}%
...
\begin{itemize}%
\item Do not use the number of \emph{generations} in your EA as time measure! Instead count the FEs\only<-1>{.}\uncover<2->{, because:}%
\item<2-> The ``number of generations'' are not really comparable for different population sizes or with algorithms that do not use populations.%
\item<3-> Often, the mapping between generations and FEs is not clear\uncover<4->{, for example%
\begin{itemize}%
\item Do you evaluate offspring solutions that are identical to their parents?%
\item<5-> Is a local search involved that refines some or all solutions in the population?%
\item<6-> In a \mbox{$(\mu+\lambda)$-EA}, is the first population of size~$\mu+\lambda$, $\lambda$, or~$\mu$?%
\item<7-> What if the population size changes adaptively?%
\end{itemize}%
}%
%
\item<8-> I suggest to prefer FEs over generations if you want to count algorithm steps.%
%
\end{itemize}%
\end{frame}%
%
...
\frametitle{Runtime}%
\begin{itemize}%
\item I suggest to always measure both the consumed FEs and the runtime in milliseconds.%
...
\frametitle{Views on Performance}%
\begin{itemize}%
\item Which one is the ``better'' view on performance?%
\uncover<2->{%
\begin{enumerate}%
...
\begin{itemize}%
\item Measures the time needed to reach a target function value allows meaningful statements such as ``Algorithm~$A$ is two/ten/hundred times faster than Algorithm~$B$ in solving this problem.''%
...
\item<4-> ``Benchmarking Theory Perspective''
\end{itemize}%
}%
%
...
\end{itemize}%
%
\locateGraphic{5-}{width=0.5\paperwidth}{graphics/performance_indicators_cuts/performance_indicators_cuts_goal_not_reached}{0.25}{0.66}%
%
\end{frame}%
%
\begin{frame}%
...
\item<5-> ``How good is the tour for the TSP that we can find in 5~minutes with our algorithm?''
...
\item<3-> If we can actually always solve the problem to a ``natural'' goal quality (e.g., to optimality), then we should prefer the horizontal cut (time-to-target) method.%
...
%
\begin{itemize}%
\item For each \alert<1>{optimization problem} (like the TSP) there are several \alert<1>{instances} (e.g., different sets of cities that need to be visited).%
\uncover<2->{%
\begin{itemize}%
...
\begin{itemize}%
\item Performance values cannot be given absolute!%
...
%
\locateGraphic{5-6}{width=0.3\paperwidth}{graphics/dice/dice}{0.666}{0.58}%
\locateGraphic{7-16}{width=0.15\paperwidth}{graphics/dice/dice}{0.83}{0.76}%
\locateGraphic{17-}{width=0.25\paperwidth}{graphics/dice/dice}{0.70}{0.65}%
%
\end{frame}%
%
...
\item<4-> Three of the most common options for doing so, for estimating the ``center'' of a distribution, are to either compute the \alert{arithmetic mean}, the \alert{median}, or the \alert{geometric mean}.%
\end{itemize}%
\end{frame}%
%
\begin{frame}%
...
\uncover<2->{ Its is computed on data sample $A=(\arrayIndex{a}{0},\arrayIndex{a}{1},\dots,\arrayIndex{a}{n-1})$ as the sum of all $n$ elements $\arrayIndex{a}{i}$ in the sample data~$A$ divided by the total number~$n$ of values.%
\uncover<3->{
\begin{equation}%
...
\end{itemize}%
\locateGraphic{5}{width=0.95\paperwidth}{graphics/outlier/outlier_first_fe_time}{0.025}{0.385}%
\end{frame}%
%
\begin{frame}%
...
\item<5-> (Unless your objective function is noisy, e.g., if you measure some physical quantity, or the objective function involves randomized simulations, there are hardly any other ``outside'' effects that could mess up our results!)%
\end{enumerate}}%
...
\item<8-> Yes, also in your code! (Btw: Please use unit tests.)%
\end{itemize}}%
\item<9-> Or: \alert{bad (but rare) worst-case behaviors of our algorithm!}\uncover<10->{\\[5pt]~~\parbox{0.9\linewidth}{Imagine that:%
\uncover<11->{ Your algorithm can actually solve the TSP or MaxSat problem in polynomial time on 90\% of all instances{\dots}%
...
\includegraphics[width=\linewidth]{graphics/maxsat_hoos_poly/maxsat_hoos_poly}\\%
{\small{(Taken from the paper \emph{``Local Search Algorithms for {SAT}: An Empirical Evaluation''} by Hoos and Stützle, coloring added manually\cite{HS2000LSAFSAEE}.)%
}}}}}}{0.14}{0.14}%
%
\end{frame}%
%
...
\only<-20>{\item<18-> But often the measured numbers ``look messier'' and are harder to compare at first glance.}%
\only<-21>{\item<19-> So often we want to normalize them by picking one algorithm as ``standard'' and dividing them by its measurements.}%
...
\only<-29>{\item<23-> If we now compute the arithmetic mean\uncover<24->{, then \textcolor{green}{$A_1$ is best}\uncover<25->{ and \textcolor{red}{$A_3$ looks worst}.}}}%
\only<-29>{\item<26-> According to the median\uncover<27->{, \textcolor{green}{$A_3$ is best}\uncover<28->{ and \textcolor{red}{$A_2$ is worst!}}}}%
...
\only<-39>{\item<33-> If we now compute the arithmetic mean\uncover<34->{, then \textcolor{green}{$A_2$ is best}\uncover<35->{ and \textcolor{red}{$A_1$ looks worst}.}}}%
\only<-39>{\item<36-> According to the median\uncover<37->{, \textcolor{green}{$A_1$ is best}\uncover<38->{ and \textcolor{red}{$A_3$ is worst!}}}}%
...
\item<42-> For example, at least half of the papers on the Job Shop Scheduling Problem normalize the result qualities they obtain on benchmark instances with the \emph{Best Known Solutions} (BKS)\only<-42>{.}\uncover<43->{ and then compute the arithmetic mean.}%
%
\end{itemize}%
%
\locate{4-39}{%
...
\hline%
\uncover<23->{$\arithMean$:&\textcolor<24->{green}{1.00}&1.42&\textcolor<25->{red}{1.67}}\\%
\uncover<26->{$\median$:&1.00&\textcolor<28->{red}{2.00}&\textcolor<27->{green}{0.50}}\\%
...
\hline%
\uncover<33->{$\arithMean$:&\textcolor<35->{red}{1.67}&\textcolor<34->{green}{1.00}&1.42}\\%
\uncover<36->{$\median$:&\textcolor<37->{green}{0.50}&1.00&\textcolor<38->{red}{2.00}}\\%
...
\begin{itemize}%
\item if we divide result qualities by results of well-known heuristics or ``Best-Known Solutions''\uncover<11->{ or}%
...
\uncover<2->{ The variance~\sampleVarb{A} of a data sample~$A=(\arrayIndex{a}{0},\arrayIndex{a}{1}, \dots, \arrayIndex{a}{n-1})$ with $n$~observations can be estimated as:%
\begin{equation}%
\sampleVarb{A} = \frac{1}{n-1} \sum_{i=0}^{n-1} \left(\arrayIndex{a}{i} - \arithMeanb{A}\right)^2
\nonumber%
\end{equation}%
}%
\end{definition*}%
...
The statistical estimate~\sampleStdDevb{A} of the standard deviation of a data sample~$A=(\arrayIndex{a}{0},\arrayIndex{a}{1}, \dots, \arrayIndex{a}{n-1})$ with $n$~observations is the square root of the estimated variance~$\sampleVarb{A}$.
\begin{equation}%
...
\item<6-> We often consider \emph{percentiles} or write things like ``98\%~quantile'' or ``0.98~percentile'' or ``98\%~percentile'' meaning~$\pgls{sampleQuantileMark}^{98}_{100}$.%
\end{itemize}%
}%
\end{frame}%
%
...
\item<8-> A statement such as ``For this TSP~instance, our algorithm can find tours for with a length of $100\pm120$~km.'' makes little sense\dots%
\end{itemize}%
}%
\end{itemize}%
\locateGraphic{1}{width=0.6\paperwidth}{graphics/skew/skew_1}{0.2}{0.24}%
\locateGraphic{2}{width=0.6\paperwidth}{graphics/skew/skew_2}{0.2}{0.24}%
\locateGraphic{3}{width=0.6\paperwidth}{graphics/skew/skew_3}{0.2}{0.24}%
\locateGraphic{4-7}{width=0.45\paperwidth}{graphics/skew/skew_3}{0.275}{0.44}%
\end{frame}%
%
%
...
%
\item<5-> It means that one of the two algorithms is better\uncover<6->{\alert<6>{~with a certain probability}}%
%
\item<7-> If we say \emph{``$A$ is better than $B$,''} we have a certain probability~$p$ to be wrong.%
%
\item<8-> \alert{The statement \emph{``$A$ is better than $B$''} makes only sense if we can give an upper bound~$\alpha$ for the error probability~$p$!}%
%
\end{itemize}%
%
\end{frame}%
...
%
\item<2-> get a result (e.g., \emph{``The median of $A$ is bigger than the median of $B$''}) \alert{together with} an error probability~$p$ that the conclusion is wrong.%
%
\item<3-> If~$p$ is less than a significance level (upper bound) \ensuremath{\alpha}, we can accept the conclusion.%
%
...
%
\locateGraphic{2}{width=0.65\paperwidth}{graphics/coin/coin_head_tails}{0.175}{0.28}%
\locateGraphic{5}{width=0.45\paperwidth}{graphics/coin/coin_toss}{0.275}{0.4}%
%
\begin{itemize}%
...
%
\item<6-> Did I cheat? Is my coin ``fixed?'' (i.e., is your chance to win $\neq0.5$)%
...
%
\only<5-6>{\item<5-6> Use a program to test the combinations}%
%
\item<7-> There are~27 such combinations with a mean of \alert<7>{less or equal~4}.%
%
\item<8-> The probability~$p$ to observe a situation at least as extreme as~$A$ and~$B$ under~$H_0$ is thus:%
\end{itemize}%
\uncover<9->{%
\begin{equation}%
...
%
\locateListingBox{6}{%
\lstinputlisting[language=java]{code/EnumerateAtLeastAsExtremeScenarios.java}%
}{0}{0.1}{0.8}{1}%
%
\end{frame}
%
...
total probability $E$ to make error $E=1-((1-\ensuremath{\alpha})^k)$%
...
\locateGraphic{5}{width=0.9\paperwidth}{graphics/errorProb/multicomp_bonferroni_error}{0.05}{0.22}%
\locateGraphic{8}{width=0.9\paperwidth}{graphics/errorProb/multicomp_bonferroni_corrected}{0.05}{0.22}%
\end{frame}%
%
%
...
\begin{itemize}%
%
\item Literature usually reports tuples ``$(\textnormal{instance}, \textnormal{result}, \textnormal{runtime})$''%
%
...
%
\item<13-> We should have the ``whole performance curves!''\uncover<14->{ {\dots} ideally mean or median curves over several runs!}%
%
\end{itemize}%
}%
%
\locateGraphic{3}{width=0.7\paperwidth}{graphics/termination/points}{0.15}{0.24}%
\locateGraphic{7}{width=0.7\paperwidth}{graphics/termination/points}{0.15}{0.24}%
\locateGraphic{8}{width=0.7\paperwidth}{graphics/termination/lines}{0.15}{0.24}%
\locateGraphic{9}{width=0.7\paperwidth}{graphics/performance_dimensions/performance_dimensions}{0.15}{0.24}%
\locateGraphic{10}{width=0.7\paperwidth}{graphics/performance_cuts/performance_cuts}{0.15}{0.24}%
%
\end{frame}%
%
\section{Other Stuff}%
...
\item<7-> If we have two ``moving parts'' at the same time, it is hard to understand whether an algorithm is good and whether a problem is hard.%
...
\item<4-> If your experiments are time-consuming, also make sure to properly store all your results in human- and machine-readable form (ideally in a CSV format).%
...
This can be prevented by generating the sequence of random seeds with a deterministic algorithm and reporting both.}}%
\item<6-> Only the benchmark instances where the algorithm performs well are chosen.\uncover<7->{ %
Be wary of statements such as ``We now present the results of our algorithm on 10 of the TSPLib instances.'' (TSPLib has more than 100\dots)}%
...
Be wary of statements such as ``We compare our algorithm with the standard Genetic Algorithm.'' (because the SGA is usually not the state-of-the-art)}}%
\end{itemize}}%
...
\begin{enumerate}%
\item best result after fixed number of FEs/runtime%
\item<5-> number of FEs/runtime needed to get certain result%
\end{enumerate}%
}%
%
%

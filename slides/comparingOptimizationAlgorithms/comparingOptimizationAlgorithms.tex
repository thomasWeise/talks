...
\only<6-7>{\item<6-> Let's say we have a number of cities~\scale\only<-6>{.}\uncover<7->{ and a runtime as a function~$R(\scale)$ in this log-log plot.}}%
\only<8>{\item<8-> A linear function means that the runtime~$R(\scale)$ grows slowly with~\scale.}%
\only<9>{\item<9-> A quadratic function~(a straight line in log-log plots) is also OK.}%
\only<10-11>{\item<10-> A quartic function \only<-10>{$R(\scale)=\scale^4$ gets quite large for growing~\scale}\only<11>{exceeds the number of milliseconds per day at~$\scale\approx512$}.}%
\only<12-13>{\item<12-> \only<-12>{But this is nothing compared to the exponential function~$R(\scale)=1.1^{\scale}$\dots}\only<13->{A runtime of~$1.1^{\scale}$ becomes infeasible for~$\scale>512$.}}%

...
\only<6-7>{\item<6-> Let's say we have a number of cities~\scale\only<-6>{.}\uncover<7->{ and a runtime as a function~$f(\scale)$ in this log-log plot.}}%
\only<8>{\item<8-> A linear function means that the runtime~$f(\scale)$ grows slowly with~\scale.}%
\only<9>{\item<9-> A quadratic function~(a straight line in log-log plots) is also OK.}%
\only<10-11>{\item<10-> A quartic function \only<-10>{$f(\scale)=\scale^4$ gets quite large for growing~\scale}\only<11>{exceeds the number of milliseconds per day at~$\scale\approx512$}.}%
\only<12-13>{\item<12-> \only<-12>{But this is nothing compared to the exponential function~$f(\scale)=1.1^{\scale}$\dots}\only<13->{A runtime of~$1.1^{\scale}$ becomes infeasible for~$\scale>512$.}}%
\only<14-15>{\item<14-> For larger bases, the runtime grows even faster.}%
\only<16-17>{\item<16-> If we would enumerate all possible tours of \scale~cities in a \gls{optTSP}, that would be~\factorial{\scale}.}%
\only<3-5,18->{%
\begin{itemize}%
\only<-5>{\item Clearly, there is (at least) one shortest tour.}%
\only<-8>{\item<5-> \only<-5>{Theory proofs that the worst-case runtime needed to find this tour grows exponentially with the number~\scale\ of cities we want to visit.\cite{LLRKS1993SASAAC,CPW1998AROMSCAAA,C1971TCOTPP,K1972RACP,A2008TLOQC}}%
\only<18->{\item Finding the best tour (what exact algorithms do) may take too long.}}%
\only<-20>{\item<19-> But we can find just \alert{some} tour very quickly.}%
\only<-21>{\item<20-> Of course the quality of that tour will be lower\only<20>{: the tour will be longer than the best one}.}%
...
\locateGraphic{17}{width=0.8\paperwidth}{\sharedDir/graphics/function_growth/function_growth}{0.1}{0.29}%
%
...
\item<12-> When applying \glsFull{algoACO} instead, each \gls{objFE} takes~$\bigOb{\scale^2}$\cite{WCLTTCMY2014BOAAOSFFTTSP}.%
\end{itemize}%
}%
\end{itemize}%
}%
...
\item<6-> Correction needed\uncover<7->{: Bonferroni correction\cite{B1935ICDASGDT,S1995MHT,MJ1981SSI}\uncover<6->{:~Use $\ensuremath{\alpha}'=\ensuremath{\alpha}/k$ as significance level instead of $\ensuremath{\alpha}$, then the overall probability $E$ to make an error will remain $E\leq\ensuremath{\alpha}$.}}%
}}}%
\item<9-> The Bonferroni correction is very conservative and not suitable when checking independent parameters\cite{P1998WWWBA}, but for comparing optimization algorithm, it does the trick.
...
Be wary of statements such as \inQuotes{We compare our algorithm with the \glsFull{algoSGA}.} (because the \glsShort{algoSGA} is usually not the \gls{SOTA})}}%
\end{itemize}}%
